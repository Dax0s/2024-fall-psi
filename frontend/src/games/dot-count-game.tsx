...
const DotCountGame = () => {
  ...
  const fetchInfo = async (maxDots: number) => {
    try {
      const response = await fetch(`http://localhost:5252/DotCountGame?maxDots=${maxDots}`);
  ...
  const handleAnswer = () => {
    ...
    if (accuracy === 100) {
      ...
      resultLabel.textContent = "So close! The actual number was: " + correctAnswer.toString() + " (" + Math.floor(accuracy) + "% accuracy)";
    } else if (accuracy >= 50) {
      resultLabel.textContent = "Not bad. The actual number was: " + correctAnswer.toString() + " (" + Math.floor(accuracy) + "% accuracy)";
    } else {
      resultLabel.textContent = "Maybe next time? The actual number was: " + correctAnswer.toString() + " (" + Math.floor(accuracy) + "% accuracy)";
    }
  ...
  return (
    ...
    <input id="maxDotsInput" className="col-start-4 col-span-1" type="number" min="1" max="1000" step="1"/>
    <label className="col-start-5 col-span-1">Duration (ms):</label>
    <input id="durationInput" className="col-start-6 col-span-1" type="number" min="50" max="10000" step="50"/>
    <button className="col-start-7 col-span-1" onClick={handleClick}>GO</button>
    </div>
    <div>
    <canvas id="dotCanvas" className="m-auto rounded" color={defaultCanvasColor}/>
    </div>
    <div className="grid grid-cols-5 gap-5 gap-y-10 place-items-center-h-screen">
    <label className="col-start-2 col-end-4">How many dots did you see?</label>
    <input id="answerInput" className="col-start-2 col-span-2" type="number" min="1" max="1000" step="1"/>
    <button className="col-start-4 col-span-1" onClick={handleAnswer}>CHECK</button>
